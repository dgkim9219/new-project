...
import styles from "../styles/tablePC.module.scss";
...
export default function Table(): JSX.Element {
  ...
  const [selectedid, setselectedid] = useState(null); // To store selected planNo
  ...
  const fetchData = async () => {
    try {
      const response2 = await axios.get("/api/Query_dgSuperEndPointTablePC");
      const modifiedData = response2.data.map((item) => {
        
        const splitData = item.Qty2 ? item.Qty2.split(',') : [];
        const formattedQty2 = splitData.map(qty2 => {
          const parsedQty2 = parseInt(qty2, 10);
          return parsedQty2.toLocaleString();
        });
        ...
        return {
          ...
          Semi2Lot2: item.Semi2Lot2 ? item.Semi2Lot2.replace(/,/g, '\n') : item.Semi2Lot2,
  ...
  const handleCellClick = (cellData, id, columnName) => {
...
const handleSave = async () => {
  ...
  const day = ('0' + startDate.getDate()).slice(-2);
    try {
        const response15 = await axios.post("/api/saveData2", {
            id: selectedid,
...
const handleSave3 = async () => {
  try {
    if (isUrgent) {
      await axios.post("/api/saveData2", {
        id: selectedid,
        columnName: 'emer',
        cellValue: "긴급"
    });
...
const AddDataRow = () => {
  ...
  const [MatCmf12, setMatCmf12] = useState('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/Query_dgMasterTableAddRow2', {
        method: 'POST',
        headers: {
    ...
    <form onSubmit={handleSubmit}>
      <div className={styles.addRow}>
        ...
        <input
          ...
          id="MatCmf1"
          value={MatCmf12}
          onChange={(e) => setMatCmf12(e.target.value)}
        />
                    ...
                    return (
                      <tr {...row.getRowProps()}>
                        {row.cells.map((cell) => (
                          <td
                            key={cell.column.id}
                            onClick={() => {handleCellClick(cell.value, row.original.id, cell.column.id);
                               }}
                            className={styles.cell}
                            style={{
                              textAlign: 
                                cell.column.id === 'Qty2' ? 'right' : 
                                cell.column.id === 'memo' ? 'left' : 'center',
                              width: 
                                cell.column.id === 'memo' ? '600px' : '100px',
                              backgroundImage: (() => {
                                ...
                                switch (cell.column.id) {
                                  case 'Semi1Lot':
                                    return row.original.Semi1LotQCR && row.original.Semi1LotQCR.includes("OK") && !row.original.Semi2LotQCR   ? "#97C1A9" :
                                      ...
                                      row.original.operCode && row.original.operCode.includes("OPR-060") ? "white" : "white";
                                  case 'Semi2Lot2':
                                    return row.original.Semi2LotQCR2 && row.original.Semi2LotQCR2.includes("OK") ? "#97C1A9" :
                                      row.original.Semi2LotQCR2 && !row.original.Semi2LotQCR2.includes("OK") && row.original.Semi2LotQCR2.includes("NG") ? "#FF968A" :
                                      !row.original.Semi2LotQCR2 && row.original.Semi2Lot2 && !row.original.Semi2Lot2.includes("-") ? "#FFFFB5" :
                                      row.original.Semi2Lot2 && row.original.Semi2Lot2.includes("2024-") ? "#ABDEE6" :
                                      !row.original.Semi2LotQCR2 && row.original.Semi2Lot2 ? "#FFFFB5" : "white";
                                  case 'halbDate':
                                    return "white"
                                  case 'memo':
                                    return "white"
